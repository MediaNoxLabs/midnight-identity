...
- [x] 1.1 Add submodule `third_party/midnight-verifiable-credential-digital-passport`
...
- [x] 1.2 Record the pinned `credential-compact-0.1.0-rc3.tgz` sha256 (fetch
  once, hash it); verify the digest matches the upstream release artifacts or
  npm integrity data

## 2. Codegen

- [x] 2.1 Extend `just codegen-vc` with the core-staging preamble: fetch the
...
- [x] 2.2 Repoint the fifth entry point to
...
- [x] 2.3 Verify the regenerated file compiles:
...
- [x] 2.4 Verify `just codegen-vc-check` passes twice in a row on a clean
  tree (its `git diff --exit-code` scope already covers the crate)

## 3. Smoke tests

- [x] 3.1 Port the fixture support (`tests/support/`) to the standalone
...
- [x] 3.2 Reconcile the three existing tests (claim-root determinism/size/
...
- [x] 3.3 Add the civil-date witness test (valid decomposition accepted;
...
- [x] 4.1 `.github/workflows/ci.yml`: add
...
- [x] 4.2 Add the `CHANGELOG.md` entry (motivation: upstream reset `754b2af`
...
- [x] 4.3 Confirm the archived `2026-09-08-add-midnight-vc-families` change
  is untouched (`git status` shows no edits under
  `openspec/changes/archive/`)

## 5. Final verification

- [x] 5.1 Full gate sweep on a clean tree: `just fmt-check lint build test`
