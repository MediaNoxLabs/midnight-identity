...
- [ ] 1.1 Add `crates/midnight-vc-families` to workspace `members` and a
...
- [ ] 1.2 Create `src/lib.rs` (license header, crate docs stating the
...
- [ ] 2.1 Extend `just codegen-vc` with the fifth entry point
...
- [ ] 2.2 Extend `codegen-vc-check`'s `git diff --exit-code` scope to
  `crates/midnight-vc-families/src/contract`; verify `just codegen-vc-check`
  passes twice in a row on a clean tree
- [ ] 2.3 Build with `cargo build -p midnight-vc-families --features
...
- [ ] 3.1 Port the deterministic digital-passport fixture from the vendored
  ...
  golden outputs
- [ ] 3.2 Write `tests/smoke.rs` gated by `#![cfg(feature = "digital-passport")]`
...
- [ ] 4.1 justfile: append `families_flags := "-p midnight-vc-families
...
- [ ] 4.2 `.github/workflows/ci.yml`: add `crates/midnight-vc-families/**`
...
- [ ] 4.3 Add the blocked-crate row to `doc/publishing.md` (blocker:
...
- [ ] 5.1 Full gate sweep on a clean tree: `just ci` (fmt-check, lint, build,
