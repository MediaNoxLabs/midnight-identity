...
use crate::{
    Arm, Element, bytes32_from_hex, circuit_dst, el_bytes, el_point, el_uint, envelope_digest, hash_below_r,
...
const BLS12_381_SCALAR_MODULUS_LE: [u8; 32] = [
    0x01, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xfe, 0x5b, 0xfe, 0xff, 0x02, 0xa4, 0xbd, 0x53, 0x05, 0xd8, 0xa1,
    0x09, 0x08, 0xd8, 0x39, 0x33, 0x48, 0x7d, 0x9d, 0x29, 0x53, 0xa7, 0xed, 0x73,
];
...
fn ground_signature_json(sig_r: &EmbeddedGroupAffine, ground: &GroundSignature) -> Result<serde_json::Value> {
    Ok(json!({
...
fn sign_grant_k256(req: &GrantSignRequest) -> Result<serde_json::Value> {
    ...
    let grant_id = grant_id_k256(&self_addr, &pk_x_le, &pk_y_le, req.envelope, &origin, req.slot)?;
    ...
    let challenge = match &args.shielded {
        None => challenge_withdraw_unshielded_with_grant_k256(&head, &args.color, args.amount, &args.recipient)?,
    ...
    let sig: Signature = sk.sign_prehash(&digest).map_err(|_| anyhow!("signing failed"))?;
...
pub fn derive_grant(req: &DeriveGrantRequest) -> Result<serde_json::Value> {
    ...
    if let Some(device) = &req.device {
        ...
        if arm == "k256" {
            ...
            out["lifecycle_signatures"] =
                sign_lifecycle_jubjub(&self_addr, &device_sk, &device_pk, &grant_id, &sd, auth_nonce)?;
        }
    }
    Ok(out)
}
...
fn sign_lifecycle_jubjub(
    ...
    let (r, sig_r) = fresh_nonce();
    let ground = grind_and_sign(device_sk, &r, |grind_nonce| {
        challenge_revoke_grant_jubjub(self_addr, &sig_r, device_pk, grant_id, auth_nonce, grind_nonce)
    })?;
    ...
    out.insert("revoke_all_grants".into(), ground_signature_json(&sig_r, &ground)?);
...
mod tests {
    ...
    fn grant_id_k1_matches_the_by_hand_derivation_over_both_envelopes_and_slots() {
        ...
        for (envelope, slot, pinned) in [
            (
                0u8,
                0u8,
                "78a01a6cfc96f71a02acaca1c3d94b1d6734c40be79d34775e757809b91b78c5",
            ),
            (
                1u8,
                0u8,
                "e055abd25d81ee7b8b41cf18477bddd594c6b3ac44fa13065d94668b207badde",
            ),
            (
                0u8,
                1u8,
                "51081f6e0f2816b7e7a7afe8da50ea112bd953b1e12d90f19b74cc42d528dead",
            ),
            (
                1u8,
                1u8,
                "2e5d554a8c7e5df7ab1f88ddd80c72469d3448d11df94ef07bfd58c7ef520a7e",
            ),
        ] {
            let via_fab = grant_id_k256(&SELF, &x_le, &y_le, envelope, &origin(), slot).unwrap();
            let parts = [
    ...
    fn object_commit_matches_the_by_hand_derivation_pinned_and_read_only() {
        let via_fab = object_commit(&SALT, &COLOR, RECIPIENT_KIND, &RECIPIENT, MAX_COIN_VALUE).unwrap();
        let parts = [
    ...
    fn spent_commit_matches_the_by_hand_derivation_at_zero_and_after_a_call() {
        for (spent, pinned) in [
            (
                0u128,
                "dab069416750cc667e22bff8ee7692711dd013951096456a30886ee7f546455c",
            ),
            (
                200u128,
                "fd25f634ad76002a01eee4c1298bf8e15f2d06936deef5373f311396a26332bb",
            ),
        ] {
    ...
    fn unshielded_grant_challenge_matches_the_by_hand_derivation() {
        let h = head();
        let via_fab = challenge_withdraw_unshielded_with_grant_k256(&h, &COLOR, AMOUNT, &RECIPIENT).unwrap();
        let parts = [
    ...
    fn every_challenge_input_changes_the_challenge() {
        let h = head();
        let base = challenge_withdraw_unshielded_with_grant_k256(&h, &COLOR, AMOUNT, &RECIPIENT).unwrap();
        ...
        assert_ne!(
            base,
            challenge_withdraw_unshielded_with_grant_k256(&next, &COLOR, AMOUNT, &RECIPIENT).unwrap()
        );
        ...
        assert_ne!(
            base,
            challenge_withdraw_unshielded_with_grant_k256(&reissued, &COLOR, AMOUNT, &RECIPIENT).unwrap()
        );
        // GR-4: every operation argument is covered.
        assert_ne!(
            base,
            challenge_withdraw_unshielded_with_grant_k256(&h, &RECIPIENT, AMOUNT, &RECIPIENT).unwrap()
        );
        assert_ne!(
            base,
            challenge_withdraw_unshielded_with_grant_k256(&h, &COLOR, AMOUNT + 1, &RECIPIENT).unwrap()
        );
        assert_ne!(
            base,
        ...
        assert_ne!(
            base,
            challenge_withdraw_unshielded_with_grant_k256(&foreign, &COLOR, AMOUNT, &RECIPIENT).unwrap()
        );
    }
    ...
    fn sign_grant_signs_the_envelope_digest_of_the_grant_challenge() {
        ...
        for (envelope, circuit, pinned_challenge, pinned_digest) in [
            ...
            let expected = if envelope == 0 {
                ...
                sha256_concat(&[b"midnight_signed_message:32:".to_vec(), challenge_bytes.to_vec()])
            };
            ...
            let sig = Signature::from_scalars(
                <[u8; 32]>::try_from(hex::decode(out["sig"]["r"].as_str().unwrap().trim_start_matches("0x")).unwrap())
                    .unwrap(),
                <[u8; 32]>::try_from(hex::decode(out["sig"]["s"].as_str().unwrap().trim_start_matches("0x")).unwrap())
                    .unwrap(),
            )
            .unwrap();
    ...
    fn lifecycle_challenges_match_the_by_hand_derivation_in_the_device_family() {
        ...
        let sd = scope_digest(&SALT, &spend_scope()).unwrap();
        let dst =
            |circuit: &str| sha256_concat(&[pad_to(64, format!("midnight:account:auth:k1:v1:{circuit}").as_bytes())]);
        ...
        let grantee = challenge_withdraw_unshielded_with_grant_k256(&head, &COLOR, AMOUNT, &RECIPIENT).unwrap();
    ...
    fn unshielded_v1_grant_challenge_matches_the_by_hand_derivation() {
        ...
        let via_fab = challenge_withdraw_unshielded_with_grant_jubjub(&h, &COLOR, AMOUNT, &RECIPIENT, GRIND).unwrap();
        let parts = [
        ...
        assert_ne!(
            via_fab,
            challenge_withdraw_unshielded_with_grant_jubjub(&h, &COLOR, AMOUNT, &RECIPIENT, GRIND + 1).unwrap()
        );
        let mut other_r = h.clone();
        other_r.sig_r = jubjub_point(4);
        assert_ne!(
            via_fab,
            challenge_withdraw_unshielded_with_grant_jubjub(&other_r, &COLOR, AMOUNT, &RECIPIENT, GRIND).unwrap()
        );
        let mut next = h.clone();
        next.record_nonce = RECORD_NONCE + 1;
        assert_ne!(
            via_fab,
            challenge_withdraw_unshielded_with_grant_jubjub(&next, &COLOR, AMOUNT, &RECIPIENT, GRIND).unwrap()
        );
        ...
        assert_ne!(
            via_fab,
            challenge_withdraw_unshielded_with_grant_k256(&k1_head, &COLOR, AMOUNT, &RECIPIENT).unwrap()
        );
    }
    ...
    fn v1_lifecycle_challenges_match_the_by_hand_derivation_in_the_device_family() {
        ...
        let sd = scope_digest(&SALT, &spend_scope()).unwrap();
        let dst =
            |circuit: &str| sha256_concat(&[pad_to(64, format!("midnight:account:auth:v1:{circuit}").as_bytes())]);

        let issue = challenge_issue_grant_jubjub(&SELF, &sig_r, &device_pk, &grant_id, &sd, 1, GRIND).unwrap();
        let parts = [
            dst("issue_grant").to_vec(),
            SELF.to_vec(),
            point_le(&sig_r),
        ...
        let revoke = challenge_revoke_grant_jubjub(&SELF, &sig_r, &device_pk, &grant_id, 2, GRIND).unwrap();
        let parts = [
            dst("revoke_grant").to_vec(),
            SELF.to_vec(),
            point_le(&sig_r),
        ...
        let revoke_all = challenge_revoke_all_grants_jubjub(&SELF, &sig_r, &device_pk, 3, GRIND).unwrap();
        let parts = [
            dst("revoke_all_grants").to_vec(),
            SELF.to_vec(),
            point_le(&sig_r),
    ...
    fn the_v1_lifecycle_and_grant_families_are_pairwise_distinct_under_equal_arguments() {
        ...
        let all = [
            challenge_issue_grant_jubjub(&SELF, &sig_r, &pk, &grant_id, &sd, nonce, GRIND).unwrap(),
            ...
            challenge_withdraw_unshielded_with_grant_jubjub(&head, &COLOR, AMOUNT, &RECIPIENT, GRIND).unwrap(),
    ...
    fn sign_grant_on_the_v1_arm_grinds_and_self_verifies_with_a_fixed_sk() {
        ...
        for circuit in GRANT_OPERATIONS {
            ...
            assert_eq!(out["challenge"].as_str().unwrap(), hex::encode(expected), "{circuit}");
        }
    ...
    fn derive_grant_signs_the_three_lifecycle_calls_on_the_v1_arm() {
        ...
        for circuit in ["issue_grant", "revoke_grant", "revoke_all_grants"] {
            ...
            let expected = match circuit {
                "issue_grant" => challenge_issue_grant_jubjub(&SELF, &sig_r, &device_pk, &grant_id, &sd, 7, grind),
                "revoke_grant" => challenge_revoke_grant_jubjub(&SELF, &sig_r, &device_pk, &grant_id, 7, grind),
    ...
    fn typescript_vectors_reproduce_from_their_own_arguments() {
        ...
        let path = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/vectors/grants-e1.json");
        let text =
            std::fs::read_to_string(path).expect("src/tests/vectors/grants-e1.json is written by `npm run test:unit`");
        let doc: Value = serde_json::from_str(&text).unwrap();

        let h32 = |v: &Value| -> [u8; 32] { hex::decode(v.as_str().unwrap()).unwrap().try_into().unwrap() };
        ...
        }
        {
            use group::Group as _;
            use midnight_curves::JubjubSubgroup;
            let scalar: u64 = doc["fixtures"]["sig_r_v1"]["scalar"].as_str().unwrap().parse().unwrap();
            let sig_r = EmbeddedGroupAffine(JubjubSubgroup::generator() * JubjubScalar::from(scalar));
        ...
        for vector in doc["vectors"].as_array().unwrap() {
            ...
            let got = match circuit {
                ...
                "derive_grant_spent_commit" => spent_commit(&h32(&a["scope_salt"]), uint(&a["spent"])),
                "derive_grant_rp_commit" => rp_commit(&h32(&a["scope_salt"]), &h32(&a["rp_id_hash"])),
                "derive_grant_scope_digest" => scope_digest(
                    ...
                    &GrantScopePlain {
                        ...
                        op_withdraw_shielded_to_contract: flag(&a["op_withdraw_shielded_to_contract"]),
                        read: flag(&a["read"]),
                        color: h32(&a["color"]),
                ...
                "challenge_withdraw_unshielded_with_grant_k256" => challenge_withdraw_unshielded_with_grant_k256(
                    &head(a),
                    &h32(&a["color"]),
                    uint(&a["amount"]),
                    &h32(&a["recipient"]),
                ...
                "challenge_withdraw_unshielded_with_grant_jubjub" => challenge_withdraw_unshielded_with_grant_jubjub(
                    &head_jubjub(a),
                    &h32(&a["color"]),
                    uint(&a["amount"]),
                    &h32(&a["recipient"]),
        ...
        for pinned in doc["signatures"].as_array().unwrap() {
            ...
            if pinned["arm"].as_str().unwrap() == "v1" {
                ...
                // grinding loop, and s.
                let a = &pinned["args"];
                let sk_v1 = JubjubScalar::from(pinned["sk"].as_str().unwrap().parse::<u64>().unwrap());
                let r_v1 = JubjubScalar::from(pinned["nonce_scalar"].as_str().unwrap().parse::<u64>().unwrap());
                ...
                let ground = grind_and_sign(&sk_v1, &r_v1, |grind_nonce| {
                    challenge_withdraw_unshielded_with_grant_jubjub(&head_v1, &color, amount, &recipient, grind_nonce)
                })
                .unwrap();
                assert_eq!(
            ...
            assert_eq!(
                normalised, sig,
                "envelope {envelope}: normalising the twin gives back (r, s)"
            );
            assert!(
                vk.verify_prehash(&digest, &normalised).is_ok(),
                "envelope {envelope}: high-S"
            );
            k1_signatures += 1;
        }
