...
/// Source metadata for the pinned Compact account-custody contract.
pub mod source {
...
/// Wallet-local pure coin-store value transformations.
pub mod coin_store;
/// Runtime-neutral inbox discovery over a read-only ledger view.
pub mod discovery;
/// Platform-neutral InboxEntry v1 sealing and opening.
pub mod inbox;
...
const JUBJUB_R_LE: [u8; 32] = [
    0xb7, 0x2c, 0xf7, 0xd6, 0x5e, 0x0e, 0x97, 0xd0, 0x82, 0x10, 0xc8, 0xcc, 0x93, 0x20, 0x68, 0xa6, 0x00, 0x3b, 0x34,
    0x01, 0x01, 0x3b, 0x67, 0x06, 0xa9, 0xaf, 0x33, 0x65, 0xea, 0xb4, 0x7d, 0x0e,
];

// ── Field-aligned encoding elements (mirror of the compact-runtime types) ───

pub struct Element {
    pub(crate) atoms: Vec<ValueAtom>,
    pub(crate) alignment: Vec<AlignmentSegment>,
}
...
pub fn el_bytes(length: u32, data: &[u8]) -> Element {
    Element {
...
pub fn el_uint(byte_length: u32, value: u128) -> Element {
    Element {
        atoms: vec![ValueAtom(strip_trailing_zeros(value.to_le_bytes().to_vec()))],
        alignment: vec![AlignmentSegment::Atom(AlignmentAtom::Bytes { length: byte_length })],
    }
}

/// JubjubPoint: two field atoms (x, y), minimal little-endian
/// (CompactTypeJubjubPoint).
pub fn el_point(p: &EmbeddedGroupAffine) -> Result<Element> {
...
pub fn persistent_hash(elements: &[Element]) -> Result<[u8; 32]> {
    ...
    let aligned = AlignedValue::new(value, alignment).ok_or_else(|| anyhow!("invalid alignment"))?;
...
pub fn circuit_dst(arm: &Arm, circuit: &str) -> Result<[u8; 32]> {
    let tag = match arm {
...
pub fn hash_below_r(hash_le: &[u8; 32]) -> bool {
    for i in (0..32).rev() {
...
pub fn jubjub_scalar_from_hex(hex_be: &str) -> Result<JubjubScalar> {
...
pub fn pk_coords_le(vk: &VerifyingKey) -> Result<([u8; 32], [u8; 32])> {
...
pub fn k256_signing_key_from_hex(hex_be: &str) -> Result<SigningKey> {
    let clean = hex_be.trim_start_matches("0x");
    let bytes = hex::decode(format!("{:0>64}", clean)).context("bad scalar hex")?;
...
pub enum Arm {
    /// Normative MIP-0013 Schnorr-over-Jubjub arm.
    Jubjub,
    /// Interim non-normative secp256k1 ECDSA arm.
    K256,
}

/// A signing request for a gated account call.
#[derive(Clone, Debug, Deserialize)]
pub struct SignRequest {
    /// Authorization arm.
    pub arm: Arm,
    /// Circuit stem, currently `withdraw_unshielded`.
    pub circuit: String,
    /// Secret scalar/key as 0x-prefixed big-endian hex.
    pub sk: String,
    /// Account contract address bytes as hex.
    pub contract_address: String,
    /// Color bytes as hex.
    pub color: String,
    /// Amount as decimal u128.
    pub amount: String,
    /// Recipient bytes as hex.
    pub recipient: String,
    /// Auth nonce as decimal u64.
    pub auth_nonce: String,
    /// k256 envelope id; ignored by Jubjub.
    #[serde(default)]
    pub envelope: u8,
}

/// The connector's mandatory signing prefix for a 32-byte payload.
pub const CONNECTOR_ENVELOPE_PREFIX: &[u8; 27] = b"midnight_signed_message:32:";

/// Digest a k256 challenge under the requested envelope id.
pub fn envelope_digest(envelope: u8, challenge: &[u8; 32]) -> Result<[u8; 32]> {
    match envelope {
        0 => persistent_hash(&[el_bytes(32, challenge)]),
        1 => persistent_hash(&[el_bytes(27, CONNECTOR_ENVELOPE_PREFIX), el_bytes(32, challenge)]),
...
pub fn sign_jubjub(req: &SignRequest) -> Result<serde_json::Value> {
...
pub fn sign_k256(req: &SignRequest) -> Result<serde_json::Value> {
    let p = call_params(req)?;
...
/// Derive the k256 device rolling entry for an account, envelope, epoch, and counter.
pub fn derive_device_entry_with_k256(
    ...
    persistent_hash(&[
        ...
        el_uint(4, u128::from(epoch)),
        el_uint(8, u128::from(counter)),
    ])
}

/// Derive the k256 boot commitment for an initial device and salt.
pub fn derive_boot_commitment_with_k256(
    salt: &[u8; 32],
    verifying_key: &VerifyingKey,
    envelope: u8,
) -> Result<[u8; 32]> {
...
/// Derive the Jubjub device rolling entry for an account, epoch, and counter.
pub fn derive_device_entry_with_jubjub(
    contract_address: &[u8; 32],
...
fn pad_tag_32(tag: &str) -> Result<[u8; 32]> {
    if tag.len() > 32 {
        bail!("tag longer than 32 bytes: {tag}");
    }
    let mut out = [0u8; 32];
...
mod tests {
    ...
    fn dst_is_sha256_of_the_padded_tag_for_both_arms() {
        let k256_by_hand = sha256_concat(&[pad_to(64, b"midnight:account:auth:k1:v1:withdraw_unshielded")]);
        assert_eq!(circuit_dst(&Arm::K256, "withdraw_unshielded").unwrap(), k256_by_hand);
        let jubjub_by_hand = sha256_concat(&[pad_to(64, b"midnight:account:auth:v1:withdraw_unshielded")]);
        assert_eq!(
    ...
    #[test]
    pub fn envelope_digests_match_by_hand_and_the_runtime() {
