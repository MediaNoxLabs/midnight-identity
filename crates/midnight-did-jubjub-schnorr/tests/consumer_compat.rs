...
fn random_nonce_uses_all_64_bytes_and_retries_zero() {
    ...
    impl rand::CryptoRng for TwoDrawRng {}
...
#[test]
fn random_nonce_accepts_dynamically_sized_crypto_rng_references() {
    trait CryptoRngCore: rand::RngCore + rand::CryptoRng {}
    impl<T: rand::RngCore + rand::CryptoRng + ?Sized> CryptoRngCore for T {}

    let mut rng = rand::rngs::OsRng;
    let rng: &mut dyn CryptoRngCore = &mut rng;
    let nonce = random_nonce_scalar(rng);
    assert_ne!(nonce, EmbeddedFr::from(0u64));
}
